...
extern uint16_t USART_BUFF;
extern float TEMP_ALERT;
...
//		USART_BUFF = USART_ReceiveData(USART2);
//		USART_SendData(USART2, USART_BUFF);
//		if(uart_state==0)
//			{
...
void EXTI9_5_IRQHandler(void){
	...
	if(EXTI_GetITStatus(EXTI_Line7) != RESET){
		bool temp = MORE_LESS_COMPARISION;
		TIM_Cmd(TIM2, ENABLE);
		while(GPIO_ReadInputDataBit(BT_PORT_A, BT_CENTER) == 0){
			if(temp != MORE_LESS_COMPARISION){
...
//GPIO_ReadInputDataBit(BT_PORT_A, BT_CENTER) == 0

void TIM2_IRQHandler(void)
{
	static int tim_state=0;
