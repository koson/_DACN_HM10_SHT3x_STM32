...
{
   unsigned int i;
   GPIO_ResetBits(LCD_PORT_CLK, LCD_PIN_CLK);                                       //  clr SCK;
   GPIO_SetBits(LCD_PORT_DIN, LCD_PIN_DIN);     																		//  set MOSI;
   for(i = 8; i > 0; i--)
   {
      if((txdata & 0x80) == 0x80) GPIO_SetBits(LCD_PORT_DIN, LCD_PIN_DIN);          //  set MOSI;
      else GPIO_ResetBits(LCD_PORT_DIN, LCD_PIN_DIN);                               //  clr MOSI;
      GPIO_SetBits(LCD_PORT_CLK, LCD_PIN_CLK);                                      //  set SCK;
      GPIO_ResetBits(LCD_PORT_CLK, LCD_PIN_CLK);                                    //  clr SCK;
      txdata <<= 1;
   }
}
...
{
   if(data_or_command) GPIO_SetBits(LCD_PORT_DC, LCD_PIN_DC);                       //  set DC;
   else GPIO_ResetBits(LCD_PORT_DC, LCD_PIN_DC);                                    //  clr DC;
   
    GPIO_ResetBits(LCD_PORT_CE, LCD_PIN_CE);                                        //  clr LCD_CS;
    shiftOut(data);                                                                 //  Send the data
    GPIO_SetBits(LCD_PORT_CE, LCD_PIN_CE);                                          //  set LCD_CS;
}
...
void LCDInit(unsigned char contrast)   //This sends the magical commands
{
   LCD_GPIO_Init();
   
   GPIO_SetBits(LCD_PORT_RST, LCD_PIN_RST);    //  set RST + LCD_CS + DC + SCK + MOSI Configure control pins
   ...
   LCDWrite(LCD_CMD, 0x10 );             	   //	 LCD bias mode;
